...
//   const CreateQuiz=()=>{
//   const url1 = 'http://localhost:3000/api/attemptQuiz'
//   const fetcher1 = async (url) => await axios.get(url).then(response=> response.data) 
// const {datas, error1} = useSWR(url1, fetcher1)
// console.log(datas)
  ...
  const onFilterSubject= async ()=>{
    const questionData = datas.filter(dat => dat.subject === sub )
    if(sub==='All Subject'){
      setData(datas)      
    }else{setData(questionData)}
  }

  const displayAllSubject = async ()=>{
   setData(datas)
   setSub('All Subject')
  }
 ...
 <>
  ...
  <option value = {sub || ' '}>{sub || "Select Option"}</option> 
  ...
  <button onClick={onFilterSubject}>Filter Subject</button> 
  <button onClick={displayAllSubject}>Display All Subjects</button> 
  {/* {sub != undefined ?  : null} */}
  </form>
  {sub ? <h1>The list of {data.length} {sub} questions available</h1> : <h1>The list of all {data.length} questions available</h1>}
  <Formik
    ...
    return(
      ...
      <FieldArray name='questions'>
      {
        fieldArrayProps =>{
          ...
          {
            questions.map(({_id, subject, questionType, question, answers, imageUrl}, i)=>(
              <div key={_id}>
             
            <div>
            <label>Check the box to select below question for the quiz</label>
            <Field name={`questions[${i}].selected`} type='checkbox' /> </div>
            <div>{imageUrl && <div> <img src={imageUrl} width='100px' height='100px' /> </div>}</div>
        ...
        <Grid item xs={6}>
          ...
          {
    quizPrepared.questions.map(({_id, subject, question, questionType, answers, author, imageUrl}, index)=>{
      return(<div key={_id}>
